...
let enumCases: [EnumCase] = entries.compactMap {
    ...
    /// Remove Deprecated Codecs (ex: ipfs) ( Looks for the word 'deprecated' in the 'description' column )
    if let desc = $0.computedProperties["description"], let d = desc as? String {
        return !d.lowercased().contains("deprecated")
    }
    return true
})
